Deno.test("reset-password-with-otp rejects an invalid request", async () => {
  const child = new Deno.Command(Deno.execPath(), { args: ["run", "--allow-net", "--allow-env", "supabase/functions/reset-password-with-otp/index.ts"], stdout: "null", stderr: "null" }).spawn();
