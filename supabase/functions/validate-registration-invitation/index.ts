...
Deno.serve({ port: Number(Deno.env.get("PORT") ?? "8000") }, async (request) => {
  ...
  if (typeof phone !== "string" || typeof invitationCode !== "string" || !PHONE_PATTERN.test(phone) || invitationCode.length === 0) {
    return invalidInvitation();
  }
  ...
  if (!secret || !url || !serviceKey) return response({ error: "Service unavailable" }, 503);
  ...
  const { data: invitation, error } = await database
    ...
    .eq("code_hash", await hashInvitationCode(invitationCode))
    .maybeSingle();
  ...
  if (!invitation || !invitation.is_active ||
    ...
    (invitation.usage_limit !== null && invitation.usage_count >= invitation.usage_limit)) {
    return invalidInvitation();
  }

  return response({
    invitationToken: await issueRegistrationToken({ invitationId: invitation.id, phone, purpose: "invite" }, secret),
  });
});
