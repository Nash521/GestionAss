Deno.test("send-password-reset-otp returns a neutral response for a valid phone", async () => {
  const child = new Deno.Command(Deno.execPath(), { args: ["run", "--allow-net", "--allow-env", "supabase/functions/send-password-reset-otp/index.ts"], stdout: "null", stderr: "null" }).spawn();
