...
create table public.contribution_payments (
  ...
  payment_reference text not null check (length(btrim(payment_reference)) > 0),
...
alter table public.disbursements
  ...
  add constraint disbursements_context_check check (
    ...
    or (disbursement_type = 'exceptional_contribution_payment' and exceptional_contribution_id is not null)
  );
...
begin
  ...
  if payment_date is null or length(btrim(coalesce(payment_reference_value, ''))) = 0 then raise exception 'Payment date and reference are required'; end if;
  begin source_value := btrim(payment_source_value)::public.contribution_payment_source; exception when invalid_text_representation then raise exception 'Invalid payment source'; end;
  ...
  values (organization_value, case when due_kind='membership' then due_id end, case when due_kind='monthly' then due_id end, case when due_kind='exceptional' then due_id end, payment_amount, source_value, btrim(payment_reference_value), payment_date, admin_id) returning id into payment_id;
  return payment_id;
end;
$$;
...
begin
  ...
  begin type_value := btrim(disbursement_type_value)::public.disbursement_type; exception when invalid_text_representation then raise exception 'Invalid disbursement type'; end;
...
begin
  ...
  if normalized_tab='monthly' then
    ...
    select coalesce(jsonb_agg(jsonb_build_object('id',id,'memberId',member_id,'month',contribution_month,'dueDate',due_date,'amountDue',amount_due,'amountPaid',amount_paid,'amountRemaining',remaining_amount,'status',status) order by contribution_month desc, due_date desc), '[]'::jsonb) into items from (select d.* from public.monthly_contribution_dues d join public.members m on m.id=d.member_id where m.organization_id=organization_value order by d.contribution_month desc,d.created_at desc offset offset_value limit limit_value) p;
    select jsonb_build_object('totalExpected',count(*) * max(o.monthly_contribution_amount)) into summary from public.members m cross join public.organizations o where m.organization_id=organization_value and m.status='active' and o.id=organization_value;
    ...
    select coalesce(jsonb_agg(jsonb_build_object('id',id,'label',label,'amount',amount,'dueDate',due_date,'createdAt',created_at) order by due_date desc,created_at desc), '[]'::jsonb) into items from (select * from public.exceptional_contributions where organization_id=organization_value order by due_date desc,created_at desc offset offset_value limit limit_value) p;
    summary := '{}'::jsonb;
  else
    select count(*) into total_count from public.disbursements where organization_id=organization_value;
    select coalesce(jsonb_agg(jsonb_build_object('id',id,'memberId',member_id,'label',label,'amount',amount,'disbursedOn',disbursed_on,'type',disbursement_type,'exceptionalContributionId',exceptional_contribution_id,'justification',justification) order by disbursed_on desc,created_at desc), '[]'::jsonb) into items from (select * from public.disbursements where organization_id=organization_value order by disbursed_on desc,created_at desc offset offset_value limit limit_value) p;
    summary := '{}'::jsonb;
  end if;
