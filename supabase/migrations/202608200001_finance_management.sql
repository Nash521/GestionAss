...
begin
 select organization_id into organization_value from public.users where id=admin_id and role='admin' and is_active;
 if organization_value is null then raise exception 'Unauthorized'; end if;
 select jsonb_build_object('monthlyAmount',monthly_contribution_amount,'dueDay',monthly_contribution_due_day) into result from public.organizations where id=organization_value;
 return result;
end; $$;
revoke all on function public.get_monthly_contribution_settings(uuid) from public;
grant execute on function public.get_monthly_contribution_settings(uuid) to service_role;
