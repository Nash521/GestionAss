...
function IdentityCard({ detail }: { detail: AdminMemberDetail }) {
  ...
  const reactivate = () => Alert.alert("Réactiver", "Confirmer la réactivation manuelle de ce membre ?", [{ text: "Annuler", style: "cancel" }, { text: "Réactiver", onPress: () => { void manageAdminMember({ memberId: member.id, action: "reactivate" }).then(() => router.back()).catch(() => Alert.alert("Action impossible", "Le changement n’a pas pu être enregistré.")); } }]);
