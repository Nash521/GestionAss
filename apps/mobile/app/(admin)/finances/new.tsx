...
export default function NewFinance() {
  ...
  const initialKind = params.kind === "monthly" ? "monthly" : params.kind === "payment" ? "payment" : params.kind === "disbursement" ? "disbursement" : "exceptional";
  const [kind, setKind] = useState<Kind | "monthly">(initialKind); const [label, setLabel] = useState(""); const [amount, setAmount] = useState("");
  ...
  const submit = async () => {
    ...
    if (kind !== "monthly" && (!label.trim() || !Number.isFinite(numeric) || numeric <= 0 || !date)) return setError("Libellé, montant et date sont obligatoires.");
    if (kind === "payment" && !dueId.trim()) return setError("La référence de cotisation est obligatoire.");
    if (kind === "monthly") { await createAdminFinanceAction({ action: "generateMonthly", month: `${date.slice(0, 7)}-01` }); router.back(); return; }
  ...
  return <ScrollView contentContainerStyle={styles.page} keyboardShouldPersistTaps="handled"><Pressable onPress={() => router.back()} accessibilityRole="button" accessibilityLabel="Retour"><Feather name="arrow-left" size={23} color="#102B3D" /></Pressable><Text style={styles.title}>Nouvelle opération</Text><Text style={styles.subtitle}>Saisissez les informations et les références nécessaires.</Text>
    <View style={styles.choices}>{(["monthly", "exceptional", "payment", "disbursement"] as const).map((value) => <Pressable key={value} onPress={() => { setKind(value); setError(""); }} style={[styles.choice, kind === value && styles.active]} accessibilityRole="button" accessibilityState={{ selected: kind === value }}><Text style={kind === value ? styles.activeText : styles.choiceText}>{value === "monthly" ? "Générer les mensualités" : value === "exceptional" ? "Cotisation exceptionnelle" : value === "payment" ? "Paiement" : "Décaissement"}</Text></Pressable>)}</View>
    {kind !== "payment" && kind !== "monthly" && <Field label="Libellé" value={label} onChangeText={setLabel} placeholder="Libellé de l'opération" />}{kind === "payment" && <Field label="Référence de cotisation" value={dueId} onChangeText={setDueId} placeholder="ID de cotisation" />}
