...
function Summary({ tab, summary }: { tab: FinanceTab; summary: Record<string, number> }) { const total = summary.total ?? summary.totalCollected ?? summary.totalAmount ?? 0; const pending = summary.pending ?? summary.totalRemaining ?? summary.remaining ?? 0; return <View style={styles.stats}><View style={styles.stat}><Text style={styles.statValue}>{money(total)}</Text><Text style={styles.statLabel}>{tab === "disbursements" ? "Total décaissé" : "Total enregistré"}</Text></View><View style={styles.stat}><Text style={styles.statValue}>{money(pending)}</Text><Text style={styles.statLabel}>{tab === "monthly" ? "Reste à encaisser" : "En attente"}</Text></View></View>; }
