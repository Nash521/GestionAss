...
import { AdminFinance, Disbursement, ExceptionalContribution, FinanceTab, MonthlyPaymentTransaction, getAdminFinance } from "../../src/lib/supabase";
...
export default function AdminFinances() {
  ...
  const load = useCallback(async (offset = 0) => {
    ...
    try {
      const result = await getAdminFinance(tab, offset, tab === "monthly" ? MONTHLY_PREVIEW_SIZE : PAGE_SIZE);
  ...
  return <View style={styles.page}><ScrollView showsVerticalScrollIndicator={false}><ImageBackground source={background} style={styles.background} imageStyle={styles.backgroundImage}><View style={styles.content}>
    ...
    {loading || (!error && loadedTab !== tab) ? <Text style={styles.message}>Chargement des finances…</Text> : error ? <View style={styles.state}><Text style={styles.message}>Impossible de charger les finances.</Text><Pressable onPress={() => void load()}><Text style={styles.retry}>Réessayer</Text></Pressable></View> : <>
      ...
      {items.length === 0 ? <Text style={styles.message}>Aucune opération pour le moment.</Text> : <View style={styles.list}>{items.map((item, index) => <View key={(item as { id?: string }).id ?? index}>{tab === "monthly" ? <MonthlyPaymentCard item={item as MonthlyPaymentTransaction} /> : <FinanceCard tab={tab} item={item as ExceptionalContribution | Disbursement} />}</View>)}{tab === "monthly" && data && data.metadata.total > MONTHLY_PREVIEW_SIZE ? <Pressable accessibilityRole="button" style={styles.more} onPress={() => router.push("/(admin)/finances/monthly-transactions")}><Text style={styles.moreText}>Voir toutes les transactions</Text></Pressable> : null}{tab !== "monthly" && data && items.length < data.metadata.total ? <Pressable style={styles.more} disabled={loadingMore} onPress={() => void load(items.length)}><Text style={styles.moreText}>{loadingMore ? "Chargement…" : "Voir plus"}</Text></Pressable> : null}</View>}
    </>}
...
function FinanceCard({ tab, item }: { tab: Exclude<FinanceTab, "monthly">; item: ExceptionalContribution | Disbursement }) {
...
Object.assign(styles, { tabs: { alignItems: "stretch", backgroundColor: "#FFF", borderColor: "#DDE6E8", borderRadius: 16, borderWidth: 1, flexDirection: "row", overflow: "hidden", shadowColor: "#102B3D", shadowOffset: { width: 0, height: 3 }, shadowOpacity: 0.12, shadowRadius: 7, elevation: 3 }, tab: { alignItems: "center", flex: 1, flexDirection: "row", gap: 6, justifyContent: "center", minWidth: 0, paddingHorizontal: 6, paddingVertical: 10 }, activeTab: { backgroundColor: "#00A99D" }, tabText: { color: "#102B3D", flexShrink: 1, fontSize: 12, fontWeight: "700", textAlign: "center" }, activeTabText: { color: "#FFF" } });
Object.assign(styles, { tabText: { fontSize: 10, fontWeight: "600" }, activeTabText: { fontWeight: "600" } });
